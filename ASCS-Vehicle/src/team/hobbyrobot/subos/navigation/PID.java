...
public class PID
{
	 ...
	 * @param d Derivative gain. Responds quickly to large changes in error. Small values prevents P and I
	 *          terms from causing overshoot.
	 */
	 ...
	 * @param d Derivative gain. Responds quickly to large changes in error. Small values prevents P and I
	 *          terms from causing overshoot.
	 * @param f Feed-forward gain. Open loop "best guess" for the output should be. Only useful if setpoint
	 *          represents a rate.
	 */
	 ...
	 * Affects output according to <b>output+=F*Setpoint</b>. Note, that a F-only system is actually open
	 * loop.
	 * 
	...
	public double getOutput(double actual, double setpoint)
	{
		...
		//SubOSController.mainLogger.log(String.format("err: %5.2f, actual: %5.2f, setpoint: %5.2f, P: %5.2f, kP: %5.2f", error, actual, setpoint, Poutput, P));
		...
		{
			setErrorSum(constrain(getErrorSum() + error, -maxError, maxError));
	...
	public void reset(boolean resetIntegral)
	{
		firstRun = true;
		if (resetIntegral)
			setErrorSum(0);
